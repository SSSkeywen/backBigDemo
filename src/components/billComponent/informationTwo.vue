<template>
  <div class="im-box">
    ...
    <ul class="im-ul">
      <li>
        <p>生效日期：</p>
        <p>{{contentData.validateDate | dateFilter}}</p>
      </li>
      <li>
        <p>主险名称：</p>
      ...
      <li>
        <p>费用发生日期：</p>
        <p>{{contentData.finishTime | dateFilter}}</p>
      </li>
    </ul>
    <div class="im-btn">
      <button
        @click="viewElectronicInvoices(contentData.policyCode,contentData.feeId,contentData.butonFlag)"
      >{{contentData.butonFlag}}</button>
    </div>
  </div>
</template>

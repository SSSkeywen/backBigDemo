<template>
  <div class="mg-box">
    <div class="annuityCollectDetail-box">
      ...
      ></headerT>
      <div
        class="annuityCollectDetail-content"
        v-for="(item, index) in annuityCollectDetail"
        :key="index"
      >
        <div class="annuityCollectDetail-content-title">
          <div class="annuityCollectDetail-content-left">保单号：{{item.policyCode}}</div>
          <div
            class="annuityCollectDetail-content-right"
            @click="selectAnnuity(index)"
        ...
        <div class="annuityCollectDetail-content-box">
          <div
            class="annuityCollectDetail-content-center"
            v-if="item.productInfo[0].productNumber"
          >险种序号： {{item.productInfo[0].productNumber}}</div>
          <div
            class="annuityCollectDetail-content-center"
            v-if="item.productInfo[0].productName"
          >险种名称： {{item.productInfo[0].productName}}</div>
          <div
            class="annuityCollectDetail-content-center"
          >保额/档次/份数： {{item.productInfo[0].amount ? item.productInfo[0].amount : 0}}&nbsp;/&nbsp;{{item.productInfo[0].benefitLevel ? item.productInfo[0].benefitLevel : 0}}&nbsp;/&nbsp;{{item.productInfo[0].unit ? item.productInfo[0].unit : 0}}</div>
          <div
            class="annuityCollectDetail-content-center"
            v-if="item.productInfo[0].initialType"
          >缴费期限： {{setinitialType(item.productInfo[0].initialType)}}</div>
          <div
            class="annuityCollectDetail-content-center"
          >开始领取年龄： {{item.productInfo[0].startPayDate === null ? 0 : item.productInfo[0].startPayDate}}</div>
          <div
            class="annuityCollectDetail-content-center"
            v-if="item.productInfo[0].payEnsure"
          >领取年限：</div>
          ...
          <div class="annuityCollectDetail-content-center" v-if="item.productInfo[0].payType">领取方式：</div>
          <select
            style="
...
export default {
  ...
  methods: {
    ...
    selectAnnuity(index) {
      this.annuityCollectDetail[index].active = !this.annuityCollectDetail[
        index
      ].active;
    },
    surenBtn() {
      ...
      if (res) {
        ...
        this.$router.push({
          path: "/statusPage",
          query: { needParams: JSON.stringify(obj) }
        });
      } else {
